...
int* MainWindow::getMatrix(QImage image)
{
    ...
    int* matrix = new int[width * 3 * height];
    QColor temp;
...
float* MainWindow::getMatrixASM(QImage image)
{
    int width = image.width();
    int height = image.height();
    float* matrix = new float[width * 3 * height];
    QColor temp;
    for (int j = 0; j < height; j++)
    {
        for (int i = 0; i < width; i++)
...
void MainWindow::on_nextWindowButton2_clicked()
{
    ...
    float* matrixASM = getMatrixASM(image);
    ...
    if (model.getMethodIndex() == 1) {
        ...
        changeToGrayscale(matrixC, 3 * image.width() * image.height(), 0);
        end_C = std::chrono::steady_clock::now();

        begin_ASM = std::chrono::steady_clock::now();
        Grayscale_GIMP_BT601(&matrixASM[0], 3 * image.width() * image.height());
        ...
        //  Lume (BT.709)
        begin_C = std::chrono::steady_clock::now();
        changeToGrayscale(matrixC, 3 * image.width() * image.height(), 1);;
        end_C = std::chrono::steady_clock::now();

        begin_ASM = std::chrono::steady_clock::now();
        Grayscale_BT709(&matrixASM[0], 3 * image.width() * image.height());
        end_ASM = std::chrono::steady_clock::now();
        ...
        //  BT.601
        begin_C = std::chrono::steady_clock::now();
        changeToGrayscale(matrixC, 3 * image.width() * image.height(), 2);
        end_C = std::chrono::steady_clock::now();

        begin_ASM = std::chrono::steady_clock::now();
        Grayscale_BT2100(&matrixASM[0], 3 * image.width() * image.height());
        end_ASM = std::chrono::steady_clock::now();
        ...
        //  Algorithmic
        begin_C = std::chrono::steady_clock::now();
        changeToGrayscale(matrixC, 3 * image.width() * image.height(), 3);
        end_C = std::chrono::steady_clock::now();

        begin_ASM = std::chrono::steady_clock::now();
        Grayscale_arithmetic(&matrixASM[0], 3 * image.width() * image.height());
        end_ASM = std::chrono::steady_clock::now();
    }
    int* matrixAsm;
    matrixAsm = new int[3 * image.width() * image.height()];
    for (int i = 0; i < (3 * image.width() * image.height()); i++)
    {
        matrixAsm[i] = (int)matrixASM[i];
    }
